...
export default function Tablet() {
  ...
  // Queries linked to the selected patient
  ...
  const handleSOS = async () => {
    ...
    if (window.confirm(`Trigger SOS for Room ${selectedPatient.roomNumber}?`)) {
      try {
        ...
        alert("SOS Signal Broadcasted! Emergency Hub notified. 🚨");
      } catch (err) {
    ...
    <div className="tablet-container">
      ...
      <main className="tablet-main">
        {selectedPatient ? (
          <div className="observation-deck">
                  ...
                  return (
                    <div key={med._id} className={`med-list-item ${isDone ? 'complete' : ''}`}>
                      ...
                      <div className="med-action-block">
                        {isDone ? <span className="given-tag">GIVEN</span> : 
            ...
            <section className="vitals-history-section">
              ...
              <table className="history-table">
                <thead><tr><th>Time</th><th>Event</th><th>Value</th><th>Staff</th></tr></thead>
                <tbody>
                  {timeline?.map((event) => (
                    <tr key={event.id}>
                      <td>{new Date(event.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</td>
                      <td><span className={`type-tag ${event.label.toLowerCase()}`}>{event.label}</span></td>
                      <td><strong>{event.value}</strong></td>
                      <td><code>{event.staff}</code></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="empty-deck">Connecting to patient records...</div>
        )}
      </main>
    </div>
  );
