...
export default function Manager() {
  ...
  console.log(" ABSENCE DATA:", emergencyAbsences);
      ...
      <div className="emergency-text-container">
        ...
        <ul className="absence-list">
          {/* {emergencyAbsences.map((abs) => (
            // <li key={abs._id}>
            //   <span className="absent-nurse-name">{abs.nurseName || "Unknown Staff"}</span>
            //   <span className="absent-details">Floor {abs.floorNumber || "?"} • {abs.shiftType || "Shift"}</span>
            // </li>
          ))} */}
        </ul>
      </div>
                  ...
                  <tr key={nurse._id}>
                    ...
                    {dates.map(date => {
                      ...
                      return (
                        <td 
                          key={date} 
                          className={`shift-cell ${shift ? 'assigned' : 'empty'} ${shift?.shiftType?.toLowerCase()}`} 
                          /* ASSIGN TRIGGER */
